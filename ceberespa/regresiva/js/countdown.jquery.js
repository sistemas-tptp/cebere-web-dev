(function ($) {
	...
	$.fn.countdown = function (options) {
		return $.fn.countdown.begin(this, $.extend({
			...
			onFinish: function () { window.location = document.referrer; }  // Executes client side when timer is zero
		}, options));
	};
	...
	$.fn.countdown.begin = function (parent, settings) {
		...
		if (!settings.init) {
			...
			$.each(timespan, function (k, v) {
				...
				$('.label', '#' + k).text((v === 1 ? $.fn.countdown.singularize(k) : $.fn.countdown.translate(k)));
			});
		}
	...
	$.fn.countdown.singularize = function (str) {
        ...
        if(str=="days"){ str="dia"; }
        if(str=="hours"){ str="hr"; }
        if(str=="minutes"){ str="min"; }
        if(str=="seconds"){ str="seg"; }
		return str;
	};
    
    $.fn.countdown.translate = function (str) {
        
        if(str=="days"){ str="dias"; }
        if(str=="hours"){ str="hrs"; }
        if(str=="minutes"){ str="min"; }
        if(str=="seconds"){ str="seg"; }
		return str;
	};
